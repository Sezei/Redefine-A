OnFire = function(plr,arg,env) -- When the command is fired, use this function. exec - Player who executed the command, args - Table of arguments. (args[2] = 2nd argument), env - Returns the main environment used.
	if (env.GetLevel(plr) < 4 and arg[2] == "@me") or (env.GetLevel(plr) < 4 and arg[2] == plr.Name) then
		...
		for _,v in pairs(game.Players:GetPlayers()) do
			if v.Name ~= plr.Name then
				if env.GetLevel(v) <= 2 then
					env.Admins.Moderators[#env.Admins.Moderators+1] = v.UserId
					adds = adds+1
				...
				if p then
					if p.Name ~= plr.Name then
						if env.GetLevel(p) < env.GetLevel(plr) then 
							env.Admins.Moderators[#env.Admins.Moderators+1] = p.UserId
						...
						return {false,"It would be better if you'd give someone ELSE to level-up, you don't want to lose Level "..env.GetLevel(plr).." permissions. Trust me."}
					end
				else
