				...
				if p then
					if p.Name ~= plr.Name then
						if env.GetLevel(p) < env.GetLevel(plr) then 
							env.templevels.admin[#env.templevels.admin+1] = p.UserId
							local isBan = env.GetLevel(p)
							env.Notify(p,"welcome","Your admin level has temporarily been set to "..isBan..".")
							return {true,"Successfully added "..p.Name.." as an temporary "..env.CurrentLanguage.Levels.Admin.."."}
						else
...
return {
	ModuleType = "Command",
	Usage = "<player>", 
	ModName = "tempadmin", -- == module.Prefix..Name
	Alias = {"tempadministrator","temp3"},
	Level = 4,
	Sandbox = false,
	Libraries = {},
	Dependencies = {},
