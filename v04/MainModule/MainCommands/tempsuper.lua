				...
				if p then
					if p.Name ~= plr.Name then
						if env.GetLevel(p) < env.GetLevel(plr) then 
							env.templevels.super[#env.templevels.super+1] = p.UserId
							local isBan = env.GetLevel(p)
							env.Notify(p,"welcome","Your admin level has temporarily been set to "..isBan..".")
							return {true,"Successfully added "..p.Name.." as an temporary "..env.CurrentLanguage.Levels.Super.."."}
						else
...
return {
	ModuleType = "Command",
	Usage = "<player>", 
	ModName = "tempsuper", -- == module.Prefix..Name
	Alias = {"tempsuperadmin","tempsadmin","temp4"},
	Level = 5,
	Sandbox = false,
	Libraries = {},
	Dependencies = {},
