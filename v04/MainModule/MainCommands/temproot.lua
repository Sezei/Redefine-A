OnFire = function(plr,arg,env) -- When the command is fired, use this function. exec - Player who executed the command, args - Table of arguments. (args[2] = 2nd argument), env - Returns the main environment used.
	...
	elseif arg[2] == "@me" then
		return {true,"...okay? there you go, free root. :eyes: (You are already Game Owner (level 6))"}
				...
				if p then
					if p.Name ~= plr.Name then
						if env.GetLevel(p) < env.GetLevel(plr) then 
							env.templevels.root[#env.templevels.root+1] = p.UserId
							local isBan = env.GetLevel(p)
							env.Notify(p,"welcome","Your admin level has temporarily been set to "..isBan..".")
							return {true,"Successfully added "..p.Name.." as an temporary "..env.CurrentLanguage.Levels.Root.."."}
						else
							return {false,"You can't give Root to someone higher or equal to your level."}
						end
					else
						return {false,"...okay? there you go, free root. :eyes: (You are already Game Owner (level 6))"}
					end
				else
...
return {
	ModuleType = "Command",
	Usage = "<player>", 
	ModName = "temproot", -- == module.Prefix..Name
	Alias = {"tempowner","temp5"},
	Level = 6,
	Sandbox = false,
	Libraries = {},
	Dependencies = {},
