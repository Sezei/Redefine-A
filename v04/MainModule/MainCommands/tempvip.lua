				...
				if p then
					if p.Name ~= plr.Name then
						if env.GetLevel(p) < env.GetLevel(plr) then 
							env.templevels.vip[#env.templevels.vip+1] = p.UserId
							local isBan = env.GetLevel(p)
							env.Notify(p,"welcome","Your admin level has temporarily been set to "..isBan..".")
							return {true,"Successfully added "..p.Name.." as an temporary "..env.CurrentLanguage.Levels.VIP.."."}
						else
...
return {
	ModuleType = "Command",
	Usage = "<player>", 
	ModName = "tempvip", -- == module.Prefix..Name
	Alias = {"tempdonor","tempdonator","temp1"},
	Level = 2,
	Sandbox = false,
	Libraries = {},
	Dependencies = {},
